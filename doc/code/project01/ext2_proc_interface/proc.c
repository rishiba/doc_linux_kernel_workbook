...
int ext2_proc_print_file_details(char *filename) {
   ...
   if(path.dentry) {
	 ...
	 printk(KERN_INFO "%s::%lu::%u",filename, inode->i_ino, ei->muid);
      } else {
...
/*
 * This should be based per file system, and error checkings should 
		...
		unsigned long count, void *data)
{
	...
	printk(KERN_INFO "The possible values are PFD path\n");	
	return count;
}
